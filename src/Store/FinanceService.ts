...
class FinanceService {
  ...
  async createPayment(paymentData: {
    ...
    try {
      ...
      throw new Error(error.response?.data?.message || 'To\'lov yaratishda xatolik');
    }
  }
  ...
  async createExpense(expenseData: {
    ...
    try {
      ...
      throw new Error(error.response?.data?.message || 'Chiqim yaratishda xatolik');
    }
  }
  ...
  async getPayments(params?: {
    ...
    try {
      const response = await api.get<ApiResponse<Payment>>('/finance/payments/list', { 
        params: {
          ...params,
      ...
      throw new Error(error.response?.data?.message || 'To\'lovlar olishda xatolik');
    }
  }
  ...
  async getExpenses(params?: {
    ...
    try {
      const response = await api.get<ApiResponse<Expense>>('/finance/expenses/list', { 
        params: {
          ...params,
          limit: params?.limit || 20,
          page: params?.page || 1
      ...
      throw new Error(error.response?.data?.message || 'Chiqimlar olishda xatolik');
    }
  }
  ...
  async getOverview(params?: {
    ...
    try {
      const response = await api.get<FinanceOverview>('/finance/overview', { 
        params: {
          ...params,
          from: params?.from || new Date(new Date().getFullYear(), 0, 1).toISOString().split('T')[0],
      ...
      throw new Error(error.response?.data?.message || 'Overview olishda xatolik');
    }
  }
  ...
  async getStudentSummary(studentId: string) {
    try {
      const response = await api.get<StudentFinanceSummary>(`/finance/students/${studentId}/summary`);
      return response.data;
    } catch (error: any) {
      console.error('Student summary olishda xatolik:', error);
      throw new Error(error.response?.data?.message || 'Student summary olishda xatolik');
    }
  }
  ...
  async exportToExcel(params?: {
    ...
    try {
      ...
      throw new Error(error.response?.data?.message || 'Export qilishda xatolik');
    }
  }
}
