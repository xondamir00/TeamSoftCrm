...
export default function GroupList() {
  ...
  const fetchGroups = async () => {
    try {
      setLoading(true);
      const { data } = await api.get("/groups", { params: { page: 1, limit: 50, search } });
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-6">
        ...
        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-lg border border-slate-200 dark:border-slate-700 p-4 sm:p-6 lg:p-8 transition-colors duration-300">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white mb-1">Group Management</h1>
              <p className="text-slate-600 dark:text-slate-400 text-sm">Manage and track all groups</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-4">
            ...
            <div className="bg-gradient-to-br from-blue-50 to-blue-100 dark:from-blue-950 dark:to-blue-900 rounded-xl p-4 border border-blue-200 dark:border-blue-800 transition-colors duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-blue-700 dark:text-blue-300 text-xs sm:text-sm font-medium mb-1">Total Groups</p>
                  <p className="text-2xl sm:text-3xl font-bold text-blue-900 dark:text-blue-100">{groups.length}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-emerald-50 to-emerald-100 dark:from-emerald-950 dark:to-emerald-900 rounded-xl p-4 border border-emerald-200 dark:border-emerald-800 transition-colors duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-emerald-700 dark:text-emerald-300 text-xs sm:text-sm font-medium mb-1">Active Groups</p>
                  <p className="text-2xl sm:text-3xl font-bold text-emerald-900 dark:text-emerald-100">{activeGroups.length}</p>
                </div>
            ...
            <div className="bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-800 dark:to-slate-700 rounded-xl p-4 border border-slate-200 dark:border-slate-600 transition-colors duration-300">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-slate-700 dark:text-slate-300 text-xs sm:text-sm font-medium mb-1">Inactive Groups</p>
                  <p className="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-slate-100">{inactiveGroupsCount}</p>
                </div>
          ...
          <table className="w-full min-w-[600px]">
            ...
            <tbody>
              {loading ? (
                ...
                <tr>
                  <td colSpan={6} className="text-center py-6 dark:text-gray-400">No groups found</td>
                </tr>
              ) : (
                activeGroups.map(g => (
                  <tr key={g.id} className="border-t dark:border-gray-800 transition-all hover:bg-gray-50 dark:hover:bg-gray-800/60">
                    <td className="p-3 dark:text-gray-200">{g.name}</td>
                    <td className="p-3 dark:text-gray-200">{getRoomName(g.roomId)}</td>
                    <td className="p-3 dark:text-gray-200">{g.capacity ?? "-"}</td>
                    <td className="p-3 dark:text-gray-200">{g.daysPattern || "-"}</td>
                    <td className="p-3 dark:text-gray-200">{formatTime(g.startTime)} - {formatTime(g.endTime)}</td>
                    <td className="p-3 flex justify-end gap-2">
                      <Button size="icon" variant="outline" onClick={() => openModal(g)} className="rounded-xl dark:border-gray-700">
                        <Pencil className="w-4 h-4" />
                      </Button>
                      <Button size="icon" variant="destructive" onClick={() => setDeleteTarget(g)} className="rounded-xl">
          ...
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex justify-end z-50">
            ...
            >
              <div className="p-4 border-b dark:border-gray-800 flex items-center justify-between">
                <h2 className="text-lg font-semibold dark:text-white">{editingGroup ? "Edit Group" : "Add Group"}</h2>
                <Button variant="outline" onClick={() => setModalOpen(false)} className="rounded-xl dark:border-gray-600">
                  Close
                </Button>
              </div>
              <div className="p-4 overflow-y-auto flex-1">
                <AddGroupForm editingGroup={editingGroup} onSuccess={() => { setModalOpen(false); fetchGroups(); }} />
              </div>
            </motion.div>
          </div>
        )}
        ...
        {deleteTarget && (
          <div className="fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            ...
            >
              <h2 className="text-lg font-semibold dark:text-white mb-2">Delete Group</h2>
              <p className="dark:text-gray-300 mb-4">Are you sure you want to delete "{deleteTarget.name}"?</p>
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setDeleteTarget(null)} className="rounded-xl dark:border-gray-600">
                  Cancel
                </Button>
